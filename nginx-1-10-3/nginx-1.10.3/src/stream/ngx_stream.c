...
ngx_stream_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_stream_conf_ctx_t **) conf = ctx;
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_stream_block] ngx_stream_max_module:%d", ngx_stream_max_module);
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_stream_block] ngx_conf_parse cf ");
