...
ngx_preinit_modules(void)
{
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        //ngx_log_error(NGX_LOG_ERR, ngx_cycle->log, 0, " [ngx_preinit_modules] ngx_modules i:%d, name:%s", i, ngx_modules[i]->name);
    }
...
ngx_count_modules(ngx_cycle_t *cycle, ngx_uint_t type)
{
    ...
    //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_count_modules] type:%d", type);
    for (i = 0; cycle->modules[i]; i++) {
        ...
        //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_count_modules] module name:%s, ctx_index:%d", module->name, module->ctx_index);
        ...
        //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_count_modules] 22 module name:%s, ctx_index:%d, next:%d", module->name, module->ctx_index, next);
        if (module->ctx_index > max) {
            max = module->ctx_index;
        }
     ...
     */
    //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_count_modules] cycle old_cycle:%p", cycle->old_cycle);
    ...
    char **order)
{
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_add_module] modules_n:%d", cf->cycle->modules_n);
    ...
    if (module->type == NGX_CORE_MODULE) {
        ...
        ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_add_module] index:%d, name:%s, create_conf:%p", module->index, module->name, core_module->create_conf);
...
again:
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_module_ctx_index] cycle module name:%s, ctx_index:%d, index:%d", module->name, module->ctx_index, index);
    ...
    /* check previous cycle */
    //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_module_ctx_index] old_cycle:%p, old_cycle modules:%p", cycle->old_cycle, cycle->old_cycle->modules);
    if (cycle->old_cycle && cycle->old_cycle->modules) {
        ...
        for (i = 0; cycle->old_cycle->modules[i]; i++) {
            ...
            //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_module_ctx_index] old_cycle module name:%s, ctx_index:%d, index:%d", module->name, module->ctx_index, index);
            if (module->ctx_index == index) {
                index++;
                goto again;
            }
