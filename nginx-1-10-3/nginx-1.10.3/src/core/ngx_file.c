...
ngx_create_paths(ngx_cycle_t *cycle, ngx_uid_t user)
{
    ...
    //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_init_cycle] entry paths nelts:%d", cycle->paths.nelts);
    for (i = 0; i < cycle->paths.nelts; i++) {
        //ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " [ngx_init_cycle] entry i:%d, path name:%s", i,  path[i]->name.data);
