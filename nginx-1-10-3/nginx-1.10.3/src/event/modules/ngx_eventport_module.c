    ...
    ngx_uint_t flags)
{
    ...
    for (i = 0; i < events; i++) {
            ...
            if (revents & POLLIN) {
                ...
                if (flags & NGX_POST_EVENTS) {
                    ...
                    ngx_log_error(NGX_LOG_ERR, cycle->log, 0,
                        "http [ngx_eventport_process_events] entry ngx_http_process_request_line");
        ...
        case PORT_SOURCE_USER:
            ngx_log_error(NGX_LOG_ERR, cycle->log, 0, " ngx_eventport_process_events Entry handler ev:%p", ev);
            ev->handler(ev);
