...
ngx_epoll_notify_handler(ngx_event_t *ev)
{
    ...
    ngx_log_error(NGX_LOG_ERR, ev->log, 0, " ngx_epoll_notify_handler Entry handler ev:%p", ev);
    handler(ev);
}
...
//nginx监听采用的是LT模式，其他采用的是ET模式。水平模式相比边缘模式来说，频繁调用触发会产生更多的系统调用，开销相比自然就高了
/* 处理已准备就绪的事件 */
static ngx_int_t
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    //                 "ngx_epoll_process_events entry");
    ...
    for (i = 0; i < events; i++) {
        ...
        if ((revents & EPOLLIN) && rev->active) {
            ...
            if (flags & NGX_POST_EVENTS) {
                ...
                ngx_log_error(NGX_LOG_ERR, cycle->log, 0,
                    "ngx_epoll_process_events entry handler rev:%p", rev);
