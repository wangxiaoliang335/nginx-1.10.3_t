...
// nginx事件模块支持了多种不同操作系统的事件机制，例如：devpoll, epoll, iocp, kqueue, poll, select, win32_select等。
// 其中ngx_event_actions_t 结构体，该成员结构实现了不同事件驱动模块的具体方法。封装了IO多路复用模型的统一接口 
// 该结构体定义在文件src/event/ngx_event.h 中：
typedef struct {
 ...
 */
#define NGX_USE_ONESHOT_EVENT    0x00000002   //单次触发

/*
 * The event filter notifies only the changes and an initial level:
 * kqueue, epoll.
 */
#define NGX_USE_CLEAR_EVENT      0x00000004   //边沿触发
...
// 事件模块主要包含以下文件：
// ngx_event.c/h 事件核心模块，以及定义所有事件模块的统一接口
// ngx_event_accept.c 事件连接处理
// ngx_event_posted.c/h 队列事件相关，主要队列事件的添加，删除，处理
// ngx_event_timer.c/h 定时器事件相关，定时器事件相关的执行，添加，删除
// 其他就是不同系统对应模块的事件epoll，poll，select，iocp，kqueue等

//nginx事件类型主要分为三类：网络io事件，定时器事件，队列事件
// nginx定义了不同的模块，有http，mail，event等模
// ngx_event_module_t 结构体是nginx事件模块的接口
