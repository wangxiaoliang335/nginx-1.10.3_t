...
ngx_ssl_handshake_handler(ngx_event_t *ev)
{
    ...
    if (ev->timedout) {
        ngx_log_error(NGX_LOG_ERR, c->log, 0, " ngx_ssl_handshake_handler Entry timedout ssl handler");
        c->ssl->handler(c);
        return;
    }
    ...
    ngx_log_error(NGX_LOG_ERR, c->log, 0, " ngx_ssl_handshake_handler Entry ssl handler");
    c->ssl->handler(c);
}
...
ngx_ssl_shutdown_handler(ngx_event_t *ev)
{
    ...
    ngx_log_error(NGX_LOG_ERR, c->log, 0, " ngx_ssl_shutdown_handler Entry handler");
    handler(c);
}
