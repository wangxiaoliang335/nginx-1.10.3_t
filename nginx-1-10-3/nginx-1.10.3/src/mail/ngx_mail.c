...
ngx_mail_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *(ngx_mail_conf_ctx_t **) conf = ctx;
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_mail_block] ngx_mail_max_module:%d", ngx_mail_max_module);
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_mail_block] ngx_conf_parse cf ");
