...
ngx_http_handler(ngx_http_request_t *r)
{
    ...
    //             "http [ngx_http_handler] entry ngx_http_core_run_phases");
...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_run_phases r phase_handler:%d", r->phase_handler);
    while (ph[r->phase_handler].checker) {
        rc = ph[r->phase_handler].checker(r, &ph[r->phase_handler]);
        // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
        //         "http entry ngx_http_core_run_phases rc:%d, r phase_handler:%d", rc, r->phase_handler);

        if (rc == NGX_OK) {
            // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
            //     "http entry ngx_http_core_run_phases rc NGX_OK");
            return;
        }
    }
}
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_generic_phase");
...
ngx_http_core_rewrite_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_rewrite_phase");
    rc = ph->handler(r);

    if (rc == NGX_DECLINED) {
        r->phase_handler++;
        //ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
        //        "http entry ngx_http_core_rewrite_phase r phase_handler:%d", r->phase_handler);
        return NGX_AGAIN;
    }
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_find_config_phase");
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_post_rewrite_phase");
...
ngx_http_core_access_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_access_phase");
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_post_access_phase");
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "http entry ngx_http_core_try_files_phase");
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http entry ngx_http_core_content_phase");
    ...
    ngx_str_t *uri, ngx_str_t *args)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http [ngx_http_internal_redirect] entry ngx_http_handler");
...
ngx_http_named_location(ngx_http_request_t *r, ngx_str_t *name)
{
    ...
    if (cscf->named_locations) {
        ...
        for (clcfp = cscf->named_locations; *clcfp; clcfp++) {
            ...
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "http [ngx_http_named_location] entry ngx_http_core_run_phases");
...
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server] NGX_HTTP_MAIN_CONF_OFFSET:%d", NGX_HTTP_MAIN_CONF_OFFSET);
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server] NGX_HTTP_SRV_CONF_OFFSET:%d", NGX_HTTP_SRV_CONF_OFFSET);
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server] NGX_HTTP_LOC_CONF_OFFSET:%d", NGX_HTTP_LOC_CONF_OFFSET);
    ...
    *cscfp = cscf;
    ...
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server] ngx_conf_parse cf ");
...
ngx_http_core_location(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    if (cf->args->nelts == 3) {
        ...
        //         " nelts == 3 [ngx_http_core_location] value[0]:%s, value[1]:%s, value[2]:%s", value[0].data, mod, name->data);
        ...
        // ngx_log_error(NGX_LOG_ERR, cf->log, 0,
        //         " [ngx_http_core_location] value[0]:%s, value[1]:%s", value[0].data, value[1].data);

        name = &value[1];

        // ngx_log_error(NGX_LOG_ERR, cf->log, 0,
        //         " [ngx_http_core_location] name 0:%c", name->data[0]);
        ...
        if (name->data[0] == '=') {
            ...
            clcf->name = *name;
            // ngx_log_error(NGX_LOG_ERR, cf->log, 0,
            //     " [ngx_http_core_location] 22 name 0:%c", name->data[0]);
            if (name->data[0] == '@') {
                clcf->named = 1;
            }
        }
    ...
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_location] ngx_conf_parse cf ");
    rv = ngx_conf_parse(cf, NULL);
...
ngx_http_core_types(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_types] ngx_conf_parse cf ");
    rv = ngx_conf_parse(cf, NULL);
...
ngx_http_core_merge_srv_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_merge_srv_conf] server_names nelts:%d ", conf->server_names.nelts);
...
#endif
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_merge_srv_conf] name:%s ", name.data);
    if (name.data[0] == '.') {
        name.len--;
        name.data++;
    }
...
ngx_http_core_server_name(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server_name] server_names nelts:%d ", cscf->server_names.nelts);
...
#endif
        sn->server = cscf;
        //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server_name] value i:%d, data:%s", i, value[i].data);
        ...
        //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_server_name] sn name:%s", sn->name.data);

        if (value[i].data[0] != '~') {
            ngx_strlow(sn->name.data, sn->name.data, sn->name.len);
            //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " 11 [ngx_http_core_server_name] sn name:%s", sn->name.data);
            continue;
        }
        ...
        //ngx_log_error(NGX_LOG_ERR, cf->log, 0, " 22 [ngx_http_core_server_name] sn name:%s", sn->name.data);
        }
#else
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "using regex \"%V\" "
...
ngx_http_core_limit_except(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_core_limit_except] ngx_conf_parse cf ");
    rv = ngx_conf_parse(cf, NULL);
