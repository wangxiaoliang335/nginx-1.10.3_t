...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    if (rev->ready) {
        ...
        ngx_log_error(NGX_LOG_ERR, c->log, 0,
                "http [ngx_http_init_connection] entry ngx_http_process_request_line");
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, c->log, 0,
    //             "http [ngx_http_wait_request_handler] entry rev:%p", rev);
    ...
    // ngx_log_error(NGX_LOG_ERR, c->log, 0,
    //             "http [ngx_http_wait_request_handler] entry ngx_http_process_request_line");
...
ngx_http_ssl_handshake(ngx_event_t *rev)
{
    ...
    if (n == 1) {
        ...
        ngx_log_error(NGX_LOG_ERR, c->log, 0,
                "http [ngx_http_ssl_handshake] entry ngx_http_wait_request_handler");
...
ngx_http_ssl_handshake_handler(ngx_connection_t *c)
{
    if (c->ssl->handshaked) {
     ...
     && (defined TLSEXT_TYPE_application_layer_protocol_negotiation           \
        ...
        ngx_log_error(NGX_LOG_ERR, c->log, 0,
                "http [ngx_http_ssl_handshake_handler] entry ngx_http_wait_request_handler");
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    //             "http [ngx_http_process_request_line] entry");
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http [ngx_http_process_request_headers] entry");
...
ngx_http_process_request(ngx_http_request_t *r)
{
    ...
    // ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
    //             "http [ngx_http_process_request] entry ngx_http_handler");
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (rc == NGX_DECLINED) {
        ...
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "http [ngx_http_finalize_request] entry ngx_http_core_run_phases");
...
ngx_http_terminate_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    while (cln) {
        if (cln->handler) {
            ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "http [ngx_http_terminate_request] entry ngx_http_process_request_line");
...
ngx_http_keepalive_handler(ngx_event_t *rev)
{
    ...
    ngx_log_error(NGX_LOG_ERR, c->log, 0,
                "http [ngx_http_keepalive_handler] entry ngx_http_process_request_line");
    ...
    while (cln) {
        if (cln->handler) {
            ngx_log_error(NGX_LOG_ERR, log, 0,
                "http [ngx_http_free_request] entry ngx_http_process_request_line");
            cln->handler(cln->data);
        }
