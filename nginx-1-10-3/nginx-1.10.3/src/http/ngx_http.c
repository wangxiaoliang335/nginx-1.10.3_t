...
ngx_http_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_conf_handler] cmd name:%s, *(ngx_http_conf_ctx_t **) conf:%p", cmd->name.data, *(ngx_http_conf_ctx_t **) conf);
    ...
    *(ngx_http_conf_ctx_t **) conf = ctx;
    // ngx_http_conf_ctx_t * tmp = (ngx_http_conf_ctx_t *) conf;
    // tmp = ctx;
    // tmp = NULL;

    ngx_log_error(NGX_LOG_ERR, cf->log, 0, "11 [ngx_conf_handler] cmd name:%s, *(ngx_http_conf_ctx_t **) conf:%p", cmd->name.data, *(ngx_http_conf_ctx_t **) conf);
    /* count the number of the http modules and set up their indices */

    ngx_http_max_module = ngx_count_modules(cf->cycle, NGX_HTTP_MODULE);

    ngx_log_error(NGX_LOG_ERR, cf->log, 0, " [ngx_http_block] ngx_http_max_module:%d", ngx_http_max_module);
...
ngx_http_init_phase_handlers(ngx_conf_t *cf, ngx_http_core_main_conf_t *cmcf)
{
    ...
    //             "ngx http: entry ngx_http_init_phase_handlers");
    for (i = 0; i < NGX_HTTP_LOG_PHASE; i++) {
