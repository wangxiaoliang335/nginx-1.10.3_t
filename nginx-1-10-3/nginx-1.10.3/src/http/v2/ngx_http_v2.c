    ...
    u_char *end)
{
    ...
    ngx_log_error(NGX_LOG_ERR, fc->log, 0, " ngx_http_v2_state_rst_stream Entry handler ev:%p", ev);
    ev->handler(ev);
...
ngx_http_v2_run_request(ngx_http_request_t *r)
{
    ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                "http [ngx_http_v2_run_request] entry");
    ...
    ngx_http_v2_stream_t *stream, ngx_uint_t status)
{
    ...
    ngx_log_error(NGX_LOG_ERR, fc->log, 0,
                        "http [ngx_http_v2_terminate_stream] entry ngx_http_process_request_line");
    rev->handler(rev);
    ...
    ngx_uint_t status)
{
    ...
    for (i = 0; i < size; i++) {
        ...
        for (node = h2c->streams_index[i]; node; node = node->index) {
            ...
            ngx_log_error(NGX_LOG_ERR, c->log, 0, " ngx_http_v2_finalize_connection Entry handler ev:%p", ev);
            ev->handler(ev);
        }
    }
